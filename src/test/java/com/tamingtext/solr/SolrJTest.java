...
public class SolrJTest extends BaseDistributedSearchTestCase {
	private final transient static Log log = LogFactory.getLog(SolrJTest.class);
  ...
  */
	...
	public void setUp() throws Exception {
		...
		solr = new Http2SolrClient.Builder("http://localhost:" + port + "/solr").build();
	...
	public void doTest() throws IOException, SolrServerException {
		...
		//<start id="solrj"/>
		Http2SolrClient solr = new Http2SolrClient.Builder(
			"http://localhost:" + port + "/solr").build();//<co id="co.solrj.server"/>
		SolrInputDocument doc = new SolrInputDocument();
		doc.addField("id", "http://tortoisehare5k.tamingtext.com");//<co id="co.solrj.unique"/>
		doc.addField("mimeType", "text/plain");
		doc.addField("title",
			"Tortoise beats Hare!  Hare wants rematch."); //, 5);//<co id="co.solrj.title"/>
		Date now = new Date();
		doc.addField("date", now);//<co id="co.solrj.date"/>
    ...
    */
		...
		doc.addField("title", "Red Fox mocks Lazy Brown Dogs!");//<co id="co.solrj.title"/>
		now = new Date();
		//doc.addField("date", DateUtil.getThreadLocalDateFormat().format(now));
		doc.addField("date", now);
		...
		doc.addField("title", "Mary's Little Lamb Stolen!.");//<co id="co.solrj.title"/>
...
*/
		...
		queryParams.setMoreLikeThis(true);//<co id="solrj-search.co.mlt"/>
		queryParams.setQuery("description:number");
