...
public class WikipediaWexIndexer {
	private final transient static Logger log = LoggerFactory.getLogger(WikipediaWexIndexer.class);

	private final Http2SolrClient server;
	public static final String DEFAULT_SOLR_URL = "http://localhost:8983/solr";

	public WikipediaWexIndexer() throws MalformedURLException {
		this.server = new Http2SolrClient.Builder(DEFAULT_SOLR_URL).build();
	}

	public WikipediaWexIndexer(Http2SolrClient server) throws MalformedURLException {
		this.server = server;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			WikipediaWexIndexer indexer = new WikipediaWexIndexer(
				new Http2SolrClient.Builder(url).build());
			int total = 0;
