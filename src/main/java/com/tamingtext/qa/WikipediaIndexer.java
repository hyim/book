...
public class WikipediaIndexer {
	private final transient static Log log = LogFactory.getLog(WikipediaIndexer.class);
	...
	public WikipediaIndexer() throws MalformedURLException {
		server = new Http2SolrClient.Builder(DEFAULT_SOLR_URL).build();
	}

	public WikipediaIndexer(Http2SolrClient server) throws MalformedURLException {

		this.server = server;
	}

	private static final SimpleDateFormat formatter = new SimpleDateFormat("MM/dd/yyyy");
	private static final SimpleDateFormat solrFormatter =
	...
	public static void main(String[] args) throws Exception {
		...
		Group group = gbuilder.withName("Options")
			.withOption(wikipediaFileOpt)
			.withOption(numDocsOpt)
			.withOption(solrURLOpt)
			.withOption(solrBatchOpt)
			.create();
		...
		WikipediaIndexer indexer = new WikipediaIndexer(new Http2SolrClient.Builder(url).build());
		int total = 0;
		for (int i = 0; i < dumpFiles.length && total < numDocs; i++) {
			...
			total += totalFile;
			log.info(
				"Done Indexing: " + file + ". Indexed " + totalFile + " docs for that file and " + total + " overall.");
