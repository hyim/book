...
public class AnswerTypeContextGenerator {
	...
	private static final Pattern falseHeadsPattern = Pattern.compile(
		"^(name|type|kind|sort|form|one|breed|names|variety)$");
	...
	private final DictionaryLemmatizer wordnet;
	...
	public AnswerTypeContextGenerator(File dictDir) {
		try {
			wordnet = new DictionaryLemmatizer(dictDir);
		} catch (IOException e) {
	...
	private String[] getLemmas(Parse np) {
		...
		return wordnet.lemmatize(ArrayUtils.toArray(word), ArrayUtils.toArray("NN"));
	}
